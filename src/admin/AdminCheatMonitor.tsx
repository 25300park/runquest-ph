...
export default function AdminCheatMonitor() {
          ...
          {reports.map((report) => (
            <article key={report.id} className="rounded-lg border border-stone-800 bg-stone-950 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-black">Score {report.cheat_score}</p>
                  <p className="mt-1 text-xs text-stone-500">{report.reason ?? 'No reason stored'}</p>
                </div>
                <span className="rounded-md bg-red-950 px-2 py-1 text-xs font-black text-red-200">
                  {report.flagged ? 'flagged' : 'clear'}
                </span>
              </div>
              <p className="mt-3 text-xs text-stone-500">XP multiplier: {report.xp_multiplier}</p>
              {report.user_id && (
                <button
                  type="button"
            ...
            <article key={flag.id} className="rounded-lg border border-stone-800 bg-stone-950 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-stone-500">{flag.session_id ?? 'No session'}</p>
                </div>
